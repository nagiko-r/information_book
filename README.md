...
| ボタン | 動作 |
|---|---|
| ☆ | お気に入りに追加。先頭の「★ お気に入り」タブに集まります。もう一度押すと外れて消えます |
| ✕ | アーカイブ。もう見なくていい記事を全タブから隠します |

アーカイブした記事は、上部の「アーカイブを見る」にチェックを入れると一覧でき、
「戻す」で元に戻せます。

**保存先はブラウザ(localStorage)です。** サーバーを持たない構成のため、

- 端末・ブラウザごとの保存になります(スマホとPCでは共有されません)
- お気に入りは記事の内容ごと保存されるので、**元記事が配信から流れても残ります**
- アーカイブの記録は60日で自動的に忘れます(際限なく溜まらないように)
- ブラウザの閲覧データを消すと、お気に入りとアーカイブも消えます
...
### 攻略Wiki・買取価格の記事が邪魔なとき

Googleニュース検索は、攻略Wiki(GameWith等)の「育成論」「最強パーティ」ページや
買取価格・通販ページを大量に拾ってきます。これらは `guide_filter:` に当てはまると
**「攻略・まとめ」扱いになり、サイト上では既定で折りたたまれます**
(消えるわけではなく、ページ上部のスイッチで表示できます)。

```yaml
guide_filter:
  sources:     # この媒体のニュース記事は折りたたむ
    - GameWith
  keywords:    # タイトルにこれを含むニュース記事は折りたたむ
    - 攻略
```

読みたい媒体が埋もれてしまう場合はその行を消し、逆に邪魔な媒体が増えたら追加してください。
公式YouTube等の動画は対象外なので、常にそのまま表示されます。
